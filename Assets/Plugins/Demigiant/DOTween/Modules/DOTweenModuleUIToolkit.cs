	...
	public static class DOTweenModuleUIToolkit
    {
        ...
        public static TweenerCore<Vector2, Vector2, VectorOptions> DOMove(this VisualElement target, Vector2 endValue, float duration, bool snapping = false)
        {
            TweenerCore<Vector2, Vector2, VectorOptions> t
                = DOTween.To(() => new Vector2(target.resolvedStyle.translate.x,target.resolvedStyle.translate.y), x => target.style.translate = new Translate(x.x, x.y, 0f), endValue, duration);
            t.SetOptions(snapping).SetTarget(target);
            return t;
            //new Vector2(target.resolvedStyle.translate.x, target.resolvedStyle.translate.y), x => target.style.translate = new Translate(x.x, x.y, 0f), endValue, duration
            //(Vector2)target.resolvedStyle.translate, x => target.style.translate = x, endValue, duration
        }
